...
const Services = () => {
    ...
    useEffect(() => {
        ...
        //precisa remover o evento senão toda vez q o componente desse o mount iria abrir outro eventlistener emcima do anterior, gerando problemas de performance. Assim temos apenas um eventlistener ativo.
    ...
    return (
        <div id='services' className='min-h-screen flex flex-col justify-center items-center px-4 xl:pb-12 py-14'>
                ...
                {skillCards.map((card, index) => (
                    ...
                    >
                        <div className='p-3 bg-gray-200 dark:bg-gray-800 transition-colors duration-500'>
                            ...
                            <h3 className='md:text-2xl text-xl font-bold my-4 text-red-400 dark:text-yellow-500 transition-colors duration-500'>{card.title}</h3>
                            <p className='text-gray-900 dark:text-white md:h-28 h-24 md:text-base text-sm font-light overflow-y-auto overflow-x-hidden custom-scrollbar pr-2 transition-colors duration-500'>{card.description}</p>
                        </div>
                        <div className={`w-full absolute left-0 ${getPositionClass(card)} flex flex-col gap-y-5 py-4 transition-all duration-300 -z-10 ${hoveredCardIndex === index && `${getHoverPositionClass(card)}`}`}>
                            {isLargeScreen && card.hoverPosition.large === 'top' && (
                                <div className='flex justify-center'>
                                    {[...Array(card.projectCount)].map((project, index) => (
                                        <a href={card.projectLink} target='_blank' key={index} className='text-lg bg-red-500 dark:bg-yellow-500 w-10 aspect-square grid place-items-center text-white rounded-full transition-colors duration-500'>{index + 1}</a>
                                    ))}
                                </div>
                            )}
